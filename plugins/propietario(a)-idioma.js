...
await conn.sendButton(m.chat, lenguajeGB['smsAvisoEG']() + idioma.text + '\n' + 'დ ```English```', wm, null, [[` 𝗠 𝗘 𝗡 𝗨`, `${usedPrefix}menu`]], fkontak, m)
  
}else if (args[0] == 'id'){
global.lenguajeGB = id
await conn.sendButton(m.chat, lenguajeGB['smsAvisoEG']() + idioma.text + '\n' + 'დ ```Bahasa Indonesia```', wm, null, [[` 𝗠 𝗘 𝗡 𝗨`, `${usedPrefix}menu`]], fkontak, m)
  
}else if (args[0] == 'ar'){
global.lenguajeGB = ar
await conn.sendButton(m.chat, lenguajeGB['smsAvisoEG']() + idioma.text + '\n' + 'დ ```عرب```', wm, null, [[` 𝗠 𝗘 𝗡 𝗨`, `${usedPrefix}menu`]], fkontak, m)
  
}else if (args[0] == 'pt'){
global.lenguajeGB = pt
await conn.sendButton(m.chat, lenguajeGB['smsAvisoEG']() + idioma.text + '\n' + 'დ ```Português```', wm, null, [[`𝗠 𝗘 𝗡 𝗨`, `${usedPrefix}menu`]], fkontak, m)
}else{
...
await m.reply(`${fg}\`\`\`Repote the command  ${usedPrefix + command} Cammand ${usedPrefix}reporte\`\`\``) 
console.log(e) 
}}
